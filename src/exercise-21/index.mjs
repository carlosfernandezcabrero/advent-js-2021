export default function canCarry (capacity, trip) {
  ...
  const lastDeliveryPoint = trip.at(-1).at(-1)

  trip.forEach((t) => {
    collectionPoints[t[1]] = t[0]
    deliveryPoints[t[2]] = t[0]
  })

  let numGifts = 0
  for (let i = 0; i <= lastDeliveryPoint; i++) {
    if (collectionPoints[i]) numGifts += collectionPoints[i]
    if (deliveryPoints[i]) numGifts -= deliveryPoints[i]

    if (numGifts > capacity) return false
  }
